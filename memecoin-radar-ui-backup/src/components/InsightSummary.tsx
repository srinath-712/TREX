...
export const InsightSummary = ({ trend }: any) => {
  ...
  return (
    <div className="rounded-xl border border-[#2a3754] bg-[#131B2F] p-4 flex flex-col shadow-lg overflow-hidden shrink-0 min-h-40">
      <div className="flex justify-between items-center mb-4 shrink-0 border-b border-slate-800 pb-2">
        <h3 className="text-slate-200 text-sm font-bold uppercase tracking-widest flex items-center gap-2">
          Insight Summary
          {trend.gemini_analysis && <span className="text-[10px] bg-indigo-500/20 text-indigo-400 px-2 py-0.5 rounded border border-indigo-500/30 tracking-tight">✨ Gemini AI</span>}
        </h3>
      ...
      <div className="flex flex-col gap-2 h-full justify-center">
        {trend.gemini_analysis ? (
          <p className="text-slate-300 text-sm leading-relaxed tracking-wide font-medium italic border-l-4 border-indigo-500 pl-4 py-1">
            "{trend.gemini_analysis.insight_summary}"
          </p>
        ) : (
          <>
